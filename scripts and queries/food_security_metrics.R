...
# Define variables -------------------------------------------------------------
# Access all GCAM regions except for Taiwan
all_regions <- unique(prj$MultipleConsumers$`food demand`$region)[-31]
...
country_id <- read.csv("./input/country_to_gcam_id.csv")
...
mder <- read.csv("./input/MDER.csv") %>%
  rename(variable = ï..variable,
         mder_units = unit)

# Pull out commonly used queries
# Get GDP shares for each scenario
# Same units as GDP_pc - million 1990$
...
fd_pc <- fd %>%
  ...
  mutate(value = (Total * 1e12) / (365 * population),
         Units.x = "kcal.capita.day") %>%
  select(-Units, -Units.y, -Total)  %>%
  group_by(scenario, region, gcam.consumer, year, Units.x) %>%
...
# Write outputs
write.csv(dietary_energy_supply, "./fs_metrics/dietary_energy_supply.csv")
write.csv(dietary_energy_supply_decile, "./fs_metrics/dietary_energy_supply_decile.csv")

# Aggregate 10 consumer results to one consumer
dietary_agg <- dietary_energy_supply_decile %>%
  filter(!(scenario %in% c("SingleConsumer", "SingleConsumer_2p6"))) %>%
  group_by(scenario, region, year) %>%
  summarize(value = sum(value) / 10,
            units = "kcal/capita/day")

write.csv(dietary_agg, "./aggregation/des.csv")
...
# Average over decile
share_diet_staples_decile <- share_dietary_from_staples %>%
  filter(!(scenario %in% c("SingleConsumer", "SingleConsumer_2p6"))) %>%
  group_by(scenario, region, gcam.consumer, year, Units) %>%
  summarize(avg_staples_in_total = mean(staples_in_total))

# Write outputs
write.csv(share_diet_staples_region, "./fs_metrics/share_des_staples_region.csv")
write.csv(share_diet_staples_decile, "./fs_metrics/share_des_staples_decile.csv")

# Aggregate 10 consumer results to one consumer
share_staples_agg <- share_diet_staples_decile %>%
  group_by(scenario, region, year) %>%
  summarize(agg_staples_in_total = sum(avg_staples_in_total) / 10)
...
# Write outputs
write.csv(adesa, "./fs_metrics/average_des_adequacy.csv")
write.csv(adesa_method2, "./fs_metrics/average_des_adequacy_percent.csv")
